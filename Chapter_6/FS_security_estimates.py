...
class FSParameterSet(object):
    def __init__(self, q, n=256, k=1, l=1, sigma=0, eta=5, security=120, alpha = 256):
        """
...
if "--record_180" in sys.argv:
    #sigma = 296.67
    #Target sigma = 275
    params += [("G+G Signature Recommended", FSParameterSet(48640, n, k=3, l=5, eta=1, security = 180, sigma = sqrt(2)*277.59, alpha = 128))]
if "--record_260" in sys.argv:
    #sigma = 331.72
    #target sigma = 265
    params += [("G+G Signature Very High", FSParameterSet(202753, n, k=4, l=7, eta=1, security=260, sigma = sqrt(2)*331.72, alpha = 1024))]
if "--record_512_ntru" in sys.argv:
    ...
    params += [("G+G NTRU Signature Medium", FSParameterSet(32257, n=512, k=1, l=1, eta=2, security = 120, sigma = sqrt(2)*443, alpha = 256))]
    params += [("G+G NTRU Signature Medium", FSParameterSet(26113, n=512, k=1, l=1, eta=2, security = 120, sigma = sqrt(2)*443, alpha = 256))]
if "--record_1024_ntru" in sys.argv:
    params += [("G+G NTRU Signature Recommended", FSParameterSet(32257, n=1024, k=1, l=1, eta=1, security = 180, sigma = sqrt(2)*368.23, alpha = 1024))]
...
print("\\\\")
print("S"+"".join([" & "+str(dps[1].sigma/(2*sqrt(log(dps[1].n-1+dps[1].n*2**65)))) for dps in params]))
print("\\\\")
